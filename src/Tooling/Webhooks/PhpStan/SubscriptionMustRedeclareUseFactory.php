...
final class SubscriptionMustRedeclareUseFactory extends Rule
{
    /**
     * @param  Class_  $node
     */
    ...
    {
        $this->error(
            message: 'A '.class_basename(Subscription::class).' subclass must redeclare #['.class_basename(UseFactory::class).']. PHP does not inherit attributes.',
            line: $node->name?->getStartLine() ?? $node->getStartLine(),
            identifier: 'webhooks.Subscription.UseFactory.required',
        );
    }
}
