...
1. Reading the `Timestamp` and `Signature` headers.
2. Computing `hash_hmac('sha256', "{$timestamp}.{$payload}", $secret)`.
3. Comparing with `hash_equals()`.
4. Rejecting if the timestamp is too old.
...
- `collecting` → `webhooks.queues.collecting` → `WEBHOOKS_QUEUE_COLLECTING`
- `sending` → `webhooks.queues.sending` → `WEBHOOKS_QUEUE_SENDING`
...
which is what happens on a timeout or a refused connection.
